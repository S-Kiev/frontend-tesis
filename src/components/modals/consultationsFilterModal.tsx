...
interface ConsultationsFilterModalProps {
  show: boolean;
...
export const ConsultationsFilterModal: FC<ConsultationsFilterModalProps> = ({ show, showModal, setFilter, filter }) => {
  const {
    handleSubmit,
    formState: { errors },
    control,
  ...
  const options = [
    ...
    { value: ConsultationStatusEnum.pending, label: 'Pendiente' },
    { value: ConsultationStatusEnum.inProgress, label: 'En progreso' },
    { value: ConsultationStatusEnum.finish, label: 'Finalizada' },
    { value: ConsultationStatusEnum.cancel, label: 'Cancelada' },
  ];
    ...
    <>
      <Modal onHide={() => showModal(false)} show={show} centered>
        <Modal.Header closeButton>
          <Modal.Title>
            <Funnel color="rgba(8, 135, 93, 1)" size={30} className="me-2" /> Filtros Consultas
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
